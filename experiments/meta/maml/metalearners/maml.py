...
from meta.maml.model import MetaFinPiMLP
import torch
...
def to_one_hot(target, nway=5, center=True):
  oh_target = target.new_zeros(target.shape[0], nway).type(torch.get_default_dtype())
...
class ModelAgnosticMetaLearning(object):
  ...
  def __init__(self, model, optimizer=None, step_size=0.1, first_order=False,
         ...
         loss_function=F.cross_entropy, device=None, no_adapt_readout=False, adapt_readout_only=False,
    ...
    self.device = device
    self.Gproj_inner = Gproj_inner
    self.Gproj_outer = Gproj_outer
  ...
  def get_outer_loss(self, batch, dobackward=True, loss_function=None, nilmax=False):
    ...
    if nilmax:
      ...
      for task_id, (train_inputs, train_targets, test_inputs, test_targets) \
        ...
        with torch.set_grad_enabled(self.model.training):
          ...
          if dobackward:
              outer_loss.backward()
              if self.Gproj_outer and hasattr(self.model, 'Gproj'):
                self.model.Gproj()
  ...
  def nilmax(self, train_inputs, train_labels, test_inputs, test_labels, regression=False, reg=1e-6):
    type(self.model).forward = MetaFinPiMLP.forward
    out = self.model.forward(train_inputs)
    nways = out.shape[-1]
    train_embed = self.model.embed.detach()
    self.model.forward(test_inputs)
    test_embed = self.model.embed.detach()

    # if True:
    #   torch.set_default_dtype(torch.float32)
    #   classifier = nn.Linear(train_embed.shape[-1], out.shape[-1]).cuda()
    #   with torch.no_grad():
    #     for p in classifier.parameters():
    #       p.zero_()
    #   train_embed = train_embed.float()
    #   test_embed = test_embed.float()
    #   optimizer = optim.Adam(classifier.parameters(), lr=0.001)
    #   for t in range(10):
    #     optimizer.zero_grad()
    #     out = classifier(train_embed)
    #     loss = F.cross_entropy(out, train_labels)
    #     # print(loss.item())
    #     loss.backward()
    #     optimizer.step()
    #   out = classifier(test_embed)
    #   argmax = torch.argmax(out, dim=-1)
    #   ncorrect = (argmax == test_labels).float().sum()
    #   torch.set_default_dtype(torch.float16)
    if regression:
      embeds = torch.cat([train_embed, test_embed]).float()
      N = len(train_labels)
      # (N, nways)
      labels = to_one_hot(train_labels, nways).float()
      ker = embeds @ embeds.T / embeds.shape[-1]
      ker += reg * torch.eye(ker.shape[0], device=ker.device, dtype=ker.dtype)
      # (N, N)
      ker_inv = torch.inverse(ker[:N, :N])
      out = ker[N:, :N] @ ker_inv @ labels
      argmax = torch.argmax(out, dim=-1)
      ncorrect = (argmax == test_labels).float().sum()
      # import pdb; pdb.set_trace()
    else:
  ...
  def adapt(self, inputs, targets, is_classification_task=None,
    ...
    for step in range(num_adaptation_steps):
      # if step >= 1:
      #   print([(name, p.norm().item()) for name, p in params.items()])
      ...
      Gproj = self.Gproj_inner
      if self.adapt_readout_only:
        Gproj = False
  ...
  def train(self, dataloader, max_batches=500, verbose=True, 
            get_all_outer_loss=False, **kwargs):
    ...
    if get_all_outer_loss:
      result_summary['all_outer_losses'] = all_outer_losses
  ...
  def train_iter(self, dataloader, max_batches=500):
    ...
    while num_batches < max_batches:
      for batch in dataloader:
        ...
        #   raise NotImplementedError('put this into the optimizer')
        #   # if self.gclip_per_param:
        #   #   for param in self.model.parameters():
        #   #     torch.nn.utils.clip_grad_norm_(param, self.grad_clip)
        #   # else:
  ...
  def evaluate(self, dataloader, max_batches=500, verbose=True, nilmax=False, **kwargs):
    mean_outer_loss, mean_accuracy, count = 0., 0., 0
    all_outer_losses = []
    with tqdm(total=max_batches, disable=not verbose, **kwargs) as pbar:
      for results in self.evaluate_iter(dataloader, max_batches=max_batches, nilmax=nilmax):
        pbar.update(1)
        count += 1
        if not nilmax:
    ...
    if not nilmax:
      ...
      result_summary = {'mean_outer_loss': -1,
                'median_outer_loss': -1}
    if 'accuracies_after' in results:
      result_summary['accuracies_after'] = mean_accuracy
  ...
  def evaluate_iter(self, dataloader, max_batches=500, nilmax=False):
    ...
    while num_batches < max_batches:
      for batch in dataloader:
        ...
        batch = tensors_to_device(batch, device=self.device)
        _, results = self.get_outer_loss(batch, dobackward=False, loss_function=self.loss_function, nilmax=nilmax)
        yield results
