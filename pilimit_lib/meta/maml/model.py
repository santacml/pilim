from inf.kernel import FinReLUGPModel
...
class MetaFinReLUGPModel(FinReLUGPModel, MetaModule):
  def __init__(self, *args, **kw):
    kw['lincls'] = SafeMetaLinear
    FinReLUGPModel.__init__(self, *args, **kw)
    self._children_modules_parameters_cache = dict()
  def forward(self, X, params=None, doreshape=True):
    if doreshape:
      X = X.reshape(X.shape[0], -1)
...
class MetaFinPiMLP(FinPiMLP, MetaModule):
  ...
  def forward(self, x, params=None):
    ...
    for l in range(1, L+1):
      if self.adapt_readout_only:
        ...
        _params = self.get_subdict(params, f'_linears.{l-1}')
      if l == 1:
        x = self.nonlin(self.first_layer_alpha * self.linears[l](x, params=_params))
      else:
        x = self.nonlin(self.linears[l](x, params=_params))
    if self.no_adapt_readout:
      _params = None
    else:
      _params = self.get_subdict(params, f'_linears.{L}')
    self.embed = x
    # if self.no_adapt_readout:
    #   return self.linears[L+1](x, params=)
    return self.linears[L+1](x, params=_params)
  ...
  def state_dict(self):
    ...
    d['adapt_readout_only'] = self.adapt_readout_only
  ...
  def load_state_dict(self, d):
    ...
    self.adapt_readout_only = d.pop('adapt_readout_only', False)
