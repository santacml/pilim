...
def get_benchmark_by_name(name,
    ...
    if name == 'sinusoid':
        ...
        if hidden_size < 0:
            model = InfPiMLP(1, 1, L=depth, r=infnet_r,
                            initbuffersize=10000, device='cpu', bias_alpha=bias_alpha, last_bias_alpha=last_bias_alpha,
                            first_layer_alpha=first_layer_alpha)
        else:
            infnet = InfPiMLP(1, 1, L=depth, r=infnet_r,
                            initbuffersize=10000, device='cpu', bias_alpha=bias_alpha, last_bias_alpha=last_bias_alpha,
                            first_layer_alpha=first_layer_alpha)
            model = infnet.sample(hidden_size, fincls=MetaFinPiMLP)
            model.cuda()
