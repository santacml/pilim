...
df = pd.read_pickle("fkr.df").reset_index()
print(df.keys())
...
for index, row in df.iterrows():
    ...
    # print(int(file[5:-3]))
    df.at[index, "ker_acc"] = float(row["ker_acc"])
    df.at[index, "loaded_width"] = str(row["loaded_width"])
    
    # if 
    # print(row["loaded_width"])

df = df.groupby(["epoch", "loaded_width"], as_index=False)["ker_acc"].max()
df = df[df["loaded_width"] != "512"]
df = df[df["loaded_width"] != "20000"]

g = sns.lineplot(x='epoch', y='ker_acc', hue='loaded_width', data=df)
plt.show()
# print(df[["width", "ker_acc", "epoch"]])
0/0
